"""Unit tests for persistent SealedDatasetStoreV11 with sealed controls."""
...
def test_sealed_dataset_store_persistent_disk_locking_and_controls():
    n_samples = 200
    dates = [f"2023-01-{i:02d}" for i in range(1, 101)] * 2
    numeric = np.random.randn(n_samples, 34)
    news = np.random.randn(n_samples, 19)
    shuffled_news = np.random.randn(n_samples, 19)
    delayed_news = np.random.randn(n_samples, 19)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 2. First unseal succeeds and delivers sealed controls
        cand_digest = "a" * 64
        test_payload = store.unseal_test_partition(cand_digest)
        assert len(test_payload.test_numeric) == 30
        assert len(test_payload.test_same_origin_shuffled_news) == 30
        assert len(test_payload.test_causal_delayed_news) == 30
