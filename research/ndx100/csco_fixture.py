"""Synthetic CSCO-like golden fixture — deterministic offline regression case.

.. warning::

   **This fixture contains no real market data.** Prices and volumes are
   generated by a deterministic simulation. Every metric derived from it is a
   *software-regression* result and says nothing about forecasting skill on
   real markets. Never quote its numbers as market performance.

This module pins network-independent inputs and expected forecasts, calendars
and metrics for a five-session retrospective benchmark over the
2026-07-20..2026-07-24 target week.
...
* **Evidence role.** This is a synthetic software-regression golden case, not
  certification-grade market evidence. See
  ``synthetic_csco_like_golden_manifest_v1.json`` and ``data/fixtures/README.md``.
"""
...
CANONICAL_CSV_NAME = "synthetic_csco_like_golden_v1.csv"
MANIFEST_NAME = "synthetic_csco_like_golden_manifest_v1.json"
...
EXPECTED_FIXTURE_ID = "synthetic_csco_like_golden_v1"
...
def load_manifest(base_path: Path | None = None) -> dict[str, Any]:
    ...
    if manifest["manifest_version"] != "1.0" or manifest["fixture_id"] != EXPECTED_FIXTURE_ID:
