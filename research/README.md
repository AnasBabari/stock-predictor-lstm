...
This research harness provides offline empirical evaluation and feature engineering for stock forecasts. Volatility research does not run in Render or the browser. The price service may import the price model definition to load an available checkpoint; research training is not invoked by that import.
...
`price_forecasting/` is retained for the learned seven-session price endpoint,
checkpoint loading and its regression tests. Volatility `model=auto` keeps the
statistical policy; the explicit G3 route uses the parity-checked frozen
QLIKE boosters with validation-panel provenance, not a new certification claim.
