...
def cmd_certify(args: argparse.Namespace) -> None:
    ...
    if not eligibility.get("universe_certification_eligible", False) or not eligibility.get(
        ...
        blocker = eligibility.get(
            "blocker", "Market or universe data is not certification-eligible."
        )
...
def main() -> int:
    ...
    p_prep = subparsers.add_parser(
        "prepare", help="Ingest data and generate immutable run manifest"
    )
    p_prep.add_argument(
        "--protocol",
        default="configs/volatility_v9_protocol.json",
        help="Path to frozen protocol JSON",
    )
    p_prep.add_argument(
        "--output-dir", default="research/results/v10-development", help="Base output directory"
    )
    ...
    p_train.add_argument(
        "--run-dir", required=True, help="Run directory containing run_manifest.json"
    )
    ...
    p_cert.add_argument(
        "--protocol", default="configs/volatility_v9_protocol.json", help="Path to protocol"
    )
