"""Comprehensive empirical study for volatility forecasting and uncertainty calibration.

Evaluates 32 diverse liquid assets across 8 sectors and 4 horizons (1, 5, 10, 20 days)
using 8 models (Persistence, Rolling 60d, EWMA, HAR-RV, Ridge, ElasticNet, Gradient Boosting,
and PyTorch LSTM).

Computes:
1. Out-of-sample MAE, RMSE, QLIKE, R2
2. Relative skill (% improvement) vs Persistence and vs HAR-RV
3. Uncertainty cone calibration (90% nominal vs empirical coverage, average width, regime breakdown)
4. Aggregate horizon tables and per-asset breakdowns.
"""
...
# 32 diverse liquid assets across 8 market sectors
TARGET_UNIVERSE = {
...
def run_study(
    ...
    print(f"Models: persistence, rolling_mean, ewma, har_rv, ridge, elastic_net, gradient_boosting" + (", lstm" if include_lstm else ""))
    ...
    output_payload = {
        "benchmark_version": "empirical-volatility-benchmark-v1",
...
def generate_markdown_report(data: dict[str, Any]) -> str:
    lines = []
    lines.append("# Empirical Volatility Forecasting Benchmark & Uncertainty Calibration Report")
    lines.append(f"**Date:** {data['date_completed']} | **Universe:** {data['universe_size']} Liquid Assets across 8 Sectors | **Execution Time:** {data['elapsed_seconds']:.1f}s\n")
    lines.append("## Executive Summary")
    lines.append("This empirical study evaluates the predictive accuracy and uncertainty calibration of 8 volatility forecasting models across 4 horizons (1-day, 5-day, 10-day, and 20-day) using strict chronological 70/15/15 splits with horizon-length boundary embargoes.\n")
    ...
    for h_key, agg_data in data["per_horizon_aggregates"].items():
        ...
        lines.append("| Model | Test MAE | Test RMSE | Test QLIKE | vs Persistence | vs HAR-RV | Val Selection Wins | Test Best Wins |")
        lines.append("| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |")
        ...
        for m, stats in models.items():
            ...
            lines.append(f"| **{name_display}** | {stats['test_mae']:.4f} | {stats['test_rmse']:.4f} | **{stats['test_qlike']:.4f}** | {vs_p} | {vs_h} | {val_wins}/{agg_data['asset_count']} | {test_wins}/{agg_data['asset_count']} |")
        lines.append("\n")

    lines.append("## 2. Uncertainty Cones & Prediction Interval Calibration")
    ...
    for h_key, agg_data in data["per_horizon_aggregates"].items():
        h = agg_data["horizon"]
        for m in ["persistence", "har_rv", "ridge", "gradient_boosting", "lstm"]:
    ...
    lines.append("### Price Diffusion Cone Coverage (Nominal Target: 90.0% p05-p95)")
    lines.append("| Horizon | Model Implied Volatility | Price Cone Empirical Coverage | Avg Cone Width (% Price) |")
    lines.append("| :---: | :--- | :---: | :---: |")
    for h_key, agg_data in data["per_horizon_aggregates"].items():
        h = agg_data["horizon"]
        for m in ["persistence", "har_rv", "gradient_boosting", "lstm"]:
...
def main() -> int:
    ...
    parser.add_argument("--tickers", default="", help="Comma-separated tickers (default: all 32)")
    parser.add_argument("--without-lstm", action="store_true", help="Skip PyTorch LSTM")
    parser.add_argument("--output-json", type=Path, default=_REPO_ROOT / "reports" / "empirical_volatility_benchmark_32_assets.json")
    parser.add_argument("--output-md", type=Path, default=_REPO_ROOT / "reports" / "empirical_volatility_benchmark_32_assets.md")
